<?php /* Smarty version 2.6.18, created on 2014-05-28 21:38:35
...
function getCreateFieldForm(modulename,blockid,mode)
{
   ...
   var type = document.getElementById("fieldType_"+blockid).value;
   var label = document.getElementById("fldLabel_"+blockid).value;
   var colname = document.getElementById("colName_"+blockid).value;
   var fldLength = document.getElementById("fldLength_"+blockid).value;  
   var fldDecimal = document.getElementById("fldDecimal_"+blockid).value;
   var fldPickList = encodeURIComponent(document.getElementById("fldPickList_"+blockid).value);
   ...
   new Ajax.Request(
		...
		{queue: {position: \'end\', scope: \'command\'},
			method: \'post\',
			postBody: \'module=Settings&action=SettingsAjax&file=LayoutBlockList&sub_mode=addNotCustomField&fld_module=\'+modulename+\'&ajax=true&blockid=\'+blockid+\'&fieldType=\'+type+\'&fldLabel=\'+label+\'&colName=\'+colname+\'&fldLength=\'+fldLength+\'&fldDecimal=\'+fldDecimal+\'&fldPickList=\'+fldPickList+\'&fldRelation=\'+fldRelation,
			onComplete: function(response) {
				VtigerJS_DialogBox.unblock();
				var str = response.responseText;
				if(str == \'ERROR\'){
