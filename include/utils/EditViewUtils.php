...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,$module_name,$mode='', $typeofdata=null)
{
	...
	else if($uitype == 2356)	//ed edited
	{
		$log->info("uitype is ".$uitype);
		if($value == '') {
			if ($fieldname != 'birthday' && $generatedtype != 2 && getTabid($module_name) != 14){
				$datetimefield = new DateTimeField( null );
				$disp_value = $datetimefield->getDisplayDateTimeValue();			
				// $disp_value = getNewDisplayDate();
			}
		} else {
			$valuearr = explode(" ",$value);
			$disp_value = getValidDisplayDate($valuearr[0])." ".$valuearr[1];
		}
		$editview_label[]=getTranslatedString($fieldlabel, $module_name);
		$date_format = parse_calendardate($app_strings['NTC_DATE_FORMAT'])." %H:%M:%S";

		$fieldvalue[] = array($disp_value => $curr_time) ;
		$fieldvalue[] = array($date_format=>"$current_user->date_format 24:00:00");
	}	//ed edited end
	else
	{
